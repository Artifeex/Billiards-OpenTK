...
namespace LearnOpenTK
{
            ...
            for (int i = 0; i < lines.Count; i++)
            {
                if (lines[i][0] == 'f')
                {
                    ...
                    for (int j = 0; j < str.Length; j++)
                    {
                        if (str[j] == "f")
                            continue;
                        vertIndex = (uint)Convert.ToInt32(str[j][0].ToString());
    ...
    public class Window : GameWindow
    {
        ...
        {
            base.OnLoad();
            BlenderImportModel box = new BlenderImportModel("E:/OpenGL/LearnOpenTK/Chapter1/3-ElementBufferObjects/cub.txt");
            _vertices = box.GetVertex().ToArray();
            _indices = box.GetIndexies().ToArray();
        ...
        // В EBO как я понимаю, уже работает с объектами как с целой точкой. Не отдельными координатами, а выбираем точки
        // которые нужны для рисовалки
